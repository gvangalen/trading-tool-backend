...
async def get_bot_portfolios(
    ...
    try:
        with conn.cursor() as cur:
            # -------------------------------------------------
            # 1) Basis bot + budget + setup (symbol)
            # -------------------------------------------------
            cur.execute(
                """
                SELECT
        ...
        for (
            ...
            # -------------------------------------------------
            # 3) Portfolio: units + cost basis
            #    (alleen filled executions tellen)
            # -------------------------------------------------
            with conn.cursor() as cur:
                cur.execute(
                    """
                ...
                units, cost_basis = cur.fetchone()
                units = float(units or 0)
                cost_basis = float(cost_basis or 0)

            avg_entry = (cost_basis / units) if units > 0 else 0

            # -------------------------------------------------
            # 4) Huidige prijs (market_data)
            # -------------------------------------------------
            with conn.cursor() as cur:
                cur.execute(
                    """
            ...
            unrealized_pnl_pct = (
                (unrealized_pnl_eur / cost_basis) * 100
                if cost_basis > 0
                else 0
            )
